	...
	<view class="modal-container" :class="{show: showValue}" @touchmove.stop @click.stop='cancel(2)'>
		<view class="modal-content" v-if="showValue" @click.stop>
			...
			<view class="modal-row">
				...
				<view class="modal-col modal-confirm" :style="confirmStyle" hover-class="modal-hover" @click='confirm' v-if="!noOk">
					{{confirmText}}
				</view>
			</view>
		</view>
...
<style lang="scss">
	...
	.modal-container{
		...
		.modal-content{
			width: 70%;border-radius: 20rpx;background: #fff;overflow: hidden;animation: fadeZoom .15s linear;
			.modal-title{
				padding: 30rpx 30rpx 0;text-align: center;color: #353535;font-size: $fontSizeLg;
			}
			.modal-title-padding{padding-bottom: 30rpx;}
			.modal-article{
				padding: 40rpx 30rpx 50rpx;font-size: $fontSizeSm;text-align: center;color: #999;
			}
			.modal-row{
